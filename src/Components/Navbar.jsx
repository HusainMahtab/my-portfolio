import React, { useState } from 'react'
import {Link} from "react-scroll"
import { IoMenuOutline } from "react-icons/io5";
import { IoClose } from "react-icons/io5";
function Navbar() {
    const [showMobileNav,setShowMobileNav]=useState(true)
  return (
    <>
    <div className='w-full flex justify-between items-center p-4 px-12 border-b-2 border-b-green-600 fixed z-20 bg-white'>
      <div className='flex gap-1 cursor-pointer animate-bounce capitalize'>
         <div className='w-10 h-8'>
            <img src="data:image/png;base64," alt="logo" className='mix-blend-multiple'/>
         </div>
         <div className='grid font-bold text-md'>
           <p>Mahta<span className='text-green-600'>b</span></p>
           <p>we<span className='text-green-600'>b</span> develope<span className='text-green-600'>r</span></p>
         </div>
      </div>
      <div className='hidden lg:block'>
         <div className='flex gap-2 font-semibold text-lg'> 
         <ul className='hidden md:flex gap-8 text-lg'>
                    <li className='cursor-pointer'>
                        <Link to="Home" smooth={true} duration={500} offset={-70} className='hover:border-b-2 hover:border-b-green-600'>Home</Link>
                    </li>
                    <li className='cursor-pointer'>
                        <Link to="About" smooth={true} duration={500} offset={-70} className='hover:border-b-2 hover:border-b-green-600'>About</Link>
                    </li>
                    <li className='cursor-pointer'>
                        <Link to="Experiance" smooth={true} duration={500} offset={-70} className='hover:border-b-2 hover:border-b-green-600'>Experiance</Link>
                    </li>
                    <li className='cursor-pointer'>
                        <Link to="Portfolio" smooth={true} duration={500} offset={-70} className='hover:border-b-2 hover:border-b-green-600'>Portfolio</Link>
                    </li>
                    <li className='cursor-pointer'>
                        <Link to="Contact" smooth={true} duration={500} offset={-70} className='hover:border-b-2 hover:border-b-green-600'>Contact</Link>
                    </li>
                </ul>
         </div>
      </div>
        <div className=' lg:hidden font-bold text-4xl text-green-600' onClick={()=>setShowMobileNav(!showMobileNav)}>
          {
             showMobileNav ? (<IoMenuOutline/>) : (<IoClose/>)
          } 
        </div>
    </div>
     {
        !showMobileNav &&(
         <div className=' w-full lg:hidden flex flex-col h-screen items-center justify-center text-green-600 gap-6 font-bold text-2xl fixed bg-white '>
         <ul className='flex flex-col items-center p-4'>
                    <li className='py-2'>
                        <Link to="Home" smooth={true} duration={500} offset={-70} onClick={() => setShowMobileNav(true)}>Home</Link>
                    </li>
                    <li className='py-2'>
                        <Link to="About" smooth={true} duration={500} offset={-70} onClick={() => setShowMobileNav(true)}>About</Link>
                    </li>
                    <li className='py-2'>
                        <Link to="Portfolio" smooth={true} duration={500} offset={-70} onClick={() => setShowMobileNav(true)}>Portfolio</Link>
                    </li>
                    <li className='py-2'>
                        <Link to="Experiance" smooth={true} duration={500} offset={-70} onClick={() => setShowMobileNav(true)}>Experiance</Link>
                    </li>
                    <li className='py-2'>
                        <Link to="Contact" smooth={true} duration={500} offset={-70} onClick={() => setShowMobileNav(true)}>Contact</Link>
                    </li>
                </ul>
         </div>
        )
     }
    <div>
      
    </div>
    </>
  )
}

export default Navbar